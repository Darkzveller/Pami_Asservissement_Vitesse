...
float encodeur_prec_D;
float encodeur_prec_G;
float vitesseG;
float vitesseD;

float Kp_vitesse_gauche = 2;
float Ki_vitesse_gauche = 0;
float Kd_vitesse_gauche = 3;

float somme_erreur_vitesse_G = 0;
float erreur_precedente_vitesse_G = 0;



float Kp_vitesse_droit = 2;
float Ki_vitesse_droit = 0;
float Kd_vitesse_droit = 3;

float somme_erreur_vitesse_D = 0;
float erreur_precedente_vitesse_D = 0;



float Te = 5;     // Delai de 5ms pour le controle
float Tc = 5;     // Delai de 5ms pour la récuperation des données des encodeurs
float Ta = 5;     // Delai de 5ms pour l'asservissement
...
void odo(void *parameters) {
  ...
  while (1) {
    ...
    vitesseG = ((countG - encodeur_prec_G) * (rayon * 2 * PI) / 2000) / Tc;
    vitesseD = ((countD - encodeur_prec_D) * (rayon * 2 * PI) / 2000) / Tc;
    // Serial.print("Count G  ");
    // Serial.print(countG);
    // Serial.print("encodeur_prec_G  ");
    // Serial.print(encodeur_prec_D);
    ...
    // Serial.print(vitesseG);
    // Serial.println(" m/s");
    encodeur_prec_G = countG;
    encodeur_prec_D = countD;
...
void controle(void *parameters) {
  ...
  while (1) {
    ...
    int PWMD = int(asservisssement_vitesse_droit(asservissement_pas_Droit(pas_droit)));
    int PWMG = int(asservisssement_vitesse_gauche(asservissement_pas_Gauche(pas_gauche)));
    Serial.println(pas_droit);
    controleMoteurDroit(PWMD);
    controleMoteurGauche(PWMG);
    // controleMoteurDroit(23);
    // controleMoteurGauche(25);
    ...
    switch (etat) {
      case 0:
        if (etat_TOR1 == HIGH) {
          etat = 1;
          // Serial.print("Bouton appuyer\n");
        }
        // controleMoteurDroit(int(asservisssement_vitesse_gauche(asservissement_pas_Droit(pas_droit))));
        // controleMoteurGauche(int(asservisssement_vitesse_gauche(asservissement_pas_Gauche(pas_gauche))));
        break;
      case 1:
...
void avancer_non_bloquant() {
  ...
  // Serial.println("non avan bloquant");
}
...
void tourner_non_bloquant() {
  ...
  controleMoteurGauche(vitesseG / 100.0 * 255.0);
  controleMoteurDroit(vitesseD / 100.0 * 255.0);
  // Serial.println("non rot bloquant");
}
...
float asservissement_pas_Gauche(float consigne) {
  ...
  // Serial.print("errreur pas gauche ");
  // Serial.println(erreur);
  float terme_proportionnel = Kp_G * erreur;
  float terme_integral = Ki_G * somme_erreur_G;
  float terme_derive = Kd_G * (erreur - erreur_precedente_G) * 1000 / Ta;
  ...
  commande = constrain(commande, -0.6, 0.6);
  // Serial.print("EG ");
  // Serial.println(erreur);
  // Serial.print("CG ");
...
float asservisssement_vitesse_gauche(float consigne) {

  float erreur = consigne - (vitesseG);
  Serial.print("Erreur  ");
  Serial.print(erreur);
  float terme_proportionnel = Kp_vitesse_gauche * erreur;
  float terme_integral = Ki_vitesse_gauche * somme_erreur_G;
  float terme_derive = Kd_vitesse_gauche * (erreur - erreur_precedente_vitesse_G) * 1000 / Ta;

  somme_erreur_vitesse_G += terme_integral;
  // Saturation de la somme
  somme_erreur_vitesse_G = constrain(somme_erreur_G, -0.1, 0.1);

  float commande = terme_proportionnel + terme_integral + terme_derive;
  // Serial.print("commande 1  ");
  // Serial.print(commande);
  erreur_precedente_vitesse_G = erreur;

  commande = constrain(commande, -100, 100);
  // Serial.print("  commande ");
  // Serial.println(commande);
  if (commande > 0) {
    commande = commande + 25;
  } else {
    commande = commande - 25;
  }
  Serial.print(" commande  ");
...
float asservissement_pas_Droit(float consigne) {
  ...
  float terme_proportionnel = Kp_D * erreur;
  float terme_integral = Ki_D * somme_erreur_D;
  float terme_derive = Kd_D * (erreur - erreur_precedente_D) * 1000 / Ta;
...
float asservisssement_vitesse_droit(float consigne) {

  float erreur = consigne - vitesseD;

  float terme_proportionnel = Kp_vitesse_droit * erreur;
  float terme_integral = Ki_vitesse_droit * somme_erreur_G;
  float terme_derive = Kd_vitesse_droit * (erreur - erreur_precedente_vitesse_D) * 1000 / Ta;

  somme_erreur_vitesse_D += terme_integral;
  // Saturation de la somme
  somme_erreur_vitesse_D = constrain(somme_erreur_G, -0.1, 0.1);

  float commande = terme_proportionnel + terme_integral + terme_derive;
  erreur_precedente_vitesse_D = erreur;
  commande = constrain(commande, -1000, 1000);
  if (commande > 0) {
    commande = commande + 25;
  } else {
    commande = commande - 25;
